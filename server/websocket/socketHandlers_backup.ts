...
class WebSocketManager {
  ...
  private broadcastToClients(clientIds: Set<string>, message: any) {
  ...
  broadcastCameraStatus(cameraId: string, status: 'online' | 'offline' | 'error', metadata?: any) {
    const subscribers = this.cameraSubscriptions.get(cameraId) || new Set();
    ...
    console.log(`Broadcasted camera status (${status}) to ${subscribers.size} clients for camera ${cameraId}`);
  }

  broadcastToCamera(cameraId: string, eventType: string, data: any) {
    const subscribers = this.cameraSubscriptions.get(cameraId) || new Set();
    ...
    this.broadcastToClients(subscribers, message);
    console.log(`Broadcasted ${eventType} to ${subscribers.size} clients for camera ${cameraId}`);
  }
